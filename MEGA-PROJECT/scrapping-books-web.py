...
for i in range(start_pages, num_pages):
    # make sure you give enough time between page loads to avoid server overload
    time.sleep(120)
    ...
    for r in rows:
        books['URL'] = [
            BASE_URL+r.find('a', attrs={'class': 'bookTitle'}).attrs['href']]
        books_df = pd.DataFrame.from_dict(books)
        books_df.to_csv('databases/books.csv', mode='a',
